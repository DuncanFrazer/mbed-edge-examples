...
EdgePTExample.prototype.registerExampleDevice = async function(deviceId) {
    ...
    return new Promise((resolve, reject) => {
        ...
        var myJson = JSON.stringify({ t : globalTemp, s : globalSetP});
        console.log(myJson);
...
EdgePTExample.prototype.updateExampleDeviceResources = async function(deviceId) {
    ...
    return new Promise((resolve, reject) => {
        ...
        var fileBuf = fs.readFileSync("temp.txt", "utf-8")
        var obj = JSON.parse(fileBuf);
        globalTemp = obj.t;
        globalSetP = obj.s;
...
//        console.log(params);
        if (received.resourcePath == '3308/0/5900') {
            globalSetP = received.value;
            console.log('new set point =', globalSetP);
        }

        console.log('preJson globalTemp', globalTemp, 'globalSetP', globalSetP);
        // react to updated value to writable resource
        var myJson = JSON.stringify({ t : globalTemp, s : globalSetP});
        console.log(myJson);
...
// watch the json file
// write some real translation logic here between json file from devices mapping onto device instances & resources of the devices
// at this stage i'm doing a basic PoC hard load of values between the file and 2 resources in a single device
// ------
// if first read then parse and build a local object, create resources
// if not first read then read, parse, compare to local object, send updated resources, update local copy
// maybe we just update local copy and send all local date on any difference
